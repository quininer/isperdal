...
def assets(this, req, res):
    return res.file(
        path.join('/path/to/your', (yield from req.rest('path')))
    )
...
def add_comment(this, req, res):
    status = (yield from db.query(
        (yield from req.rest('pid'))
    ).update(req.body))
...
def get_comment(this, req, res):
    comment = (yield from db.query(
        (yield from req.rest('pid'))
    ).query(
        (yield from req.rest('cid')).comment
    ))
